...
contract StakingPool is Initializable, UUPSUpgradeable, AccessControlUpgradeable, IERC1155Receiver {
    ...
    function addPackage(Package memory package) public onlyRole(PACKAGE_ADD_ROLE) {
        ...
        // Sum up the amounts that arrived
        uint256 sumOfAmounts = 0;
        for (uint256 i = 0; i < package.aco2_amounts.length; i++) {
            sumOfAmounts += package.aco2_amounts[i];
        }
    ...
    function claimStakingaCO2(address recipient, uint256 amount) public onlyRole(STAKING_CONTRACT_ROLE) {
        _claimStakingaCO2(recipient, amount);
    }

    function _claimStakingaCO2(address recipient, uint256 amount) internal {
